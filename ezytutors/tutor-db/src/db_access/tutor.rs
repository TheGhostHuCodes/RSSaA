...
pub async fn update_tutor_details_db(
    ...
    let new_tutor_record = Tutor {
        tutor_id: tutor_row.tutor_id,
        tutor_name: if let Some(name) = update_tutor.tutor_name {
            name
        } else {
            tutor_row.tutor_name
        },
        tutor_pic_url: if let Some(pic_url) = update_tutor.tutor_pic_url {
            pic_url
        } else {
            tutor_row.tutor_pic_url
        },
        tutor_profile: if let Some(profile) = update_tutor.tutor_profile {
            profile
        } else {
...
pub async fn delete_tutor_db(pool: &PgPool, tutor_id: i32) -> Result<String, EzyTutorError> {
    ...
    if rows_deleted == 0 {
        ...
        Ok(format!("Deleted {} records(s)", rows_deleted))
    }
}
