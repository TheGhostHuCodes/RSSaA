...
pub async fn post_new_course_db(
    ...
    let course_row = sqlx::query_as!(
        Course,
        "INSERT INTO ezy_course (tutor_id, course_name, course_description, course_duration, course_level, course_format, course_language, course_structure, course_price)
...
pub async fn delete_course_db(
    ...
    if rows_deleted == 0 {
        ...
        Ok(format!("Deleted {} record(s)", rows_deleted))
    }
}
...
pub async fn update_course_details_db(
    ...
    let description: String = if let Some(description) = update_course.course_description {
        description
    } else {
