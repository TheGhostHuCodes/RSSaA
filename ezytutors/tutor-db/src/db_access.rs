...
pub async fn get_courses_for_tutor_db(
    ...
    let course_rows = sqlx::query!("SELECT tutor_id, course_id, course_name, posted_time FROM ezy_course_c5 WHERE tutor_id = $1",
tutor_id).fetch_all(pool).await?;
...
pub async fn get_course_details_db(
    ...
    let course_row = sqlx::query!("SELECT tutor_id, course_id, course_name, posted_time FROM ezy_course_c5 WHERE tutor_id = $1 AND course_id = $2", tutor_id, course_id).fetch_one(pool).await;
